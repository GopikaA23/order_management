...
class AssigneesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create assignee" do
    assert_difference("Assignee.count") do
      post assignees_url, params: { assignee: { email: @assignee.email, name: @assignee.name } }
    end
  ...
  test "should update assignee" do
    patch assignee_url(@assignee), params: { assignee: { email: @assignee.email, name: @assignee.name } }
